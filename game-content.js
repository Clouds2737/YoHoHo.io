(function () {
  ...
  function grantEverything() {
    try {
      localStorage.setItem('coinsOwned', String(20000000));
      localStorage.setItem('playerXP', String(20000000));
      // Unlock many skins/pets by default. Game ignores missing ids safely.
      var skins = [];
      for (var i = 1; i <= 120; i += 1) skins.push(i);
      var pets = [];
      for (var j = 0; j <= 60; j += 1) pets.push(j);
      localStorage.setItem('unlockedSkins', JSON.stringify(skins));
      localStorage.setItem('unlockedPets', JSON.stringify(pets));
      localStorage.setItem('playerPet', '1');
      localStorage.setItem('playerPetLevel', '999');
    } catch (_) {}
  }
  ...
  window.YohohoContent = {
    ...
    grantEverything: grantEverything
  };
